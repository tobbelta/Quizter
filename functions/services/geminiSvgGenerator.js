...
async function generateSvgIllustration(questionData, apiKey) {
  if (!apiKey) {
    throw new Error('Gemini API key is required for SVG generation');
  }
  ...
  const prompt = `Du är en expert på att skapa tydliga, detaljerade SVG-illustrationer för quizfrågor.
...
Skapa nu en SVG-illustration för följande quizfråga:
...
Svara ENDAST med den färdiga SVG-koden, inget annat. Inga \`\`\`-block eller kommentarer.`;
  ...
  if (!text) {
    throw new Error('No text response from Gemini SVG generation');
  }
