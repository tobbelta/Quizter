...
async function generateEmoji(questionData, apiKey) {
  if (!apiKey) {
    throw new Error('Gemini API key is required for emoji generation');
  }
  ...
  const prompt = `Du är en expert på att välja DEN PERFEKTA emojin för att illustrera quizfrågor.
...
Välj DEN BÄSTA emojin för följande quizfråga:
...
Svara med EXAKT 1 emoji, inget annat. Inga ord, inga förklaringar, bara den bästa emojin.`;
  ...
  if (!text) {
    throw new Error('No text response from Gemini emoji generation');
  }

  let emojis = text.trim();
