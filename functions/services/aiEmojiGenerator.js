...
/**
 * AI-baserad Emoji-generering för att illustrera frågor
 */
const Anthropic = require('@anthropic-ai/sdk');

/**
 * Genererar emoji-illustrationer för en fråga
 * @param {object} questionData - Frågan att illustrera
 * @param {string} apiKey - Anthropic API-nyckel
 * @returns {Promise<string>} Emoji-sträng (exakt 1 emoji)
 */
async function generateEmoji(questionData, apiKey) {
  if (!apiKey) {
    throw new Error('Anthropic API key is required for emoji generation');
  }
...
Svara med EXAKT 1 emoji, inget annat. Inga ord, inga förklaringar, bara den bästa emojin.`;
  ...
  if (!textContent || !textContent.text) {
    throw new Error('No text response from AI emoji generation');
  }

  let emojis = textContent.text.trim();
