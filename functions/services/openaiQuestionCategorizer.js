...
async function categorizeQuestion(questionData, apiKey) {
  if (!apiKey) {
    throw new Error('OpenAI API key is required for categorization');
  }
...
Exempel:
{
  ...
  if (!content) {
    throw new Error('No text response from OpenAI categorization');
  }
  ...
  const ageGroups = Array.isArray(result.ageGroups) ? result.ageGroups : [];
